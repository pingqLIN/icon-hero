...
`}function Y(a){const n=a.iconPath.replace(/"/g,'\\"'),s=a.targetPaths.map(r=>r.replace(/"/g,'\\"'));return`#!/bin/bash
# macOS 圖示自動化腳本
# 生成時間: ${new Date().toLocaleString("zh-TW")}
...
read -p "按 Enter 鍵關閉"
`}function B(a){const n=a.targetPaths.map(r=>r.replace(/"/g,'\\"')),s=a.iconPath.replace(/"/g,'\\"');return`#!/bin/bash
# Linux 圖示自動化腳本
# 生成時間: ${new Date().toLocaleString("zh-TW")}
...
echo "完成！成功: $SUCCESS / 跳過: $SKIP / 失敗: $FAIL"
read -p "按 Enter 鍵關閉"
`}function J(a,n){switch(n){case"powershell":return G(a);case"applescript":return Y(a);case"bash":return B(a);default:throw new Error(`Unsupported script type: ${n}`)}}function Q(a){switch(a){case"powershell":return"ps1";case"applescript":return"scpt";case"bash":return"sh";default:return"txt"}}function X(a){switch(a){case"powershell":return`1. 右鍵點擊腳本檔案
...
3. 執行：./script.sh
4. 需要安裝 gvfs 套件（Ubuntu/Debian：sudo apt install gvfs）`;default:return"請參考對應作業系統的執行說明"}}function ee(a,n){const s=new Blob([a],{type:"text/plain"}),r=URL.createObjectURL(s),o=document.createElement("a");o.href=r,o.download=n,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(r)}const H=()=>{const a=navigator.userAgent.toLowerCase();return a.includes("win")?"powershell":a.includes("mac")?"applescript":"bash"};function ne({item:a,open:n,onOpenChange:s}){const[r,o]=t.useState(()=>H()),[u,f]=t.useState("file"),[d,g]=t.useState([]),[i,x]=t.useState("");if(t.useEffect(()=>{n&&(o(H()),f("file"),g([]),x(""))},[n,detectedType]),!a||a.status!=="completed")return null;const $=l=>{switch(l){case"powershell":return"ico";case"applescript":return"icns";case"bash":return"png"}},P=()=>{if(d.length===0){h.error("請至少新增一個目標路徑");return}const l=$(r),E=`/path/to/${a.name}.${l}`,I=J({targetPaths:d,iconPath:E,isInlineMode:u==="inline"},r);x(I),h.success("腳本已生成")},j=()=>{i&&(navigator.clipboard.writeText(i),h.success("已複製到剪貼簿"))},y=()=>{if(i){const l=Q(r);ee(i,`${a.name}_automation.${l}`),h.success("腳本已下載")}};return e.jsx(O,{open:n,onOpenChange:s,children:e.jsxs(D,{className:"max-w-3xl max-h-[90vh] overflow-y-auto",children:[e.jsxs(M,{children:[e.jsxs(Z,{className:"flex items-center gap-2",children:[e.jsx(N,{size:24,weight:"bold",className:"text-primary"}),"自動化腳本生成器"]}),e.jsx(R,{children:"生成自動化腳本以批次套用圖示至多個資料夾或檔案"})]}),e.jsx(w,{value:r,onValueChange:l=>o(l),className:"w-full",children:e.jsxs(S,{className:"grid w-full grid-cols-3",children:[e.jsxs(c,{value:"powershell",className:"gap-2",children:[e.jsx(v,{size:16,weight:"fill"}),"Windows"]}),e.jsxs(c,{value:"applescript",className:"gap-2",children:[e.jsx(W,{size:16,weight:"fill"}),"macOS"]}),e.jsxs(c,{value:"bash",className:"gap-2",children:[e.jsx(T,{size:16,weight:"fill"}),"Linux"]})]})}),r!==detectedType&&e.jsxs("div",{className:"px-3 py-2 rounded-lg bg-yellow-500/10 border border-yellow-500/30 text-yellow-600 dark:text-yellow-400 text-xs flex items-center gap-2",children:[e.jsx("span",{children:"⚠️"}),e.jsxs("span",{children:["你目前使用的是 ",e.jsx("strong",{children:detectedType==="powershell"?"Windows":detectedType==="applescript"?"macOS":"Linux"}),"， 但選擇了 ",e.jsx("strong",{children:r==="powershell"?"Windows":r==="applescript"?"macOS":"Linux"})," 腳本。 此腳本無法在你目前的系統上直接執行。"]})]}),e.jsxs("div",{className:"space-y-4",children:[e.jsxs("div",{className:"space-y-2",children:[e.jsxs("div",{className:"flex items-center justify-between",children:[e.jsx(m,{className:"text-sm font-semibold",children:"圖示檔案"}),e.jsxs(V,{variant:"secondary",className:"text-xs",children:["推薦: ",$(r).toUpperCase()]})]}),e.jsx("div",{className:"p-3 bg-secondary/20 rounded-lg border border-border",children:e.jsxs("p",{className:"text-sm font-mono text-muted-foreground",children:[a.name,".",$(r)]})})]}),e.jsxs("div",{className:"space-y-2",children:[e.jsx(m,{className:"text-sm font-semibold",children:"目標路徑"}),e.jsx(k,{paths:d,onPathsChange:g,placeholder:r==="powershell"?"C:\\Users\\Username\\Documents\\Folder":r==="applescript"?"/Users/username/Documents/Folder":"/home/username/Documents/Folder"})]}),r==="powershell"&&e.jsxs("div",{className:"flex items-center justify-between p-2.5 bg-secondary/10 rounded-lg border border-border",children:[e.jsx(m,{className:"text-xs text-muted-foreground",children:"腳本格式"}),e.jsx(w,{value:u,onValueChange:l=>f(l),className:"h-auto",children:e.jsxs(S,{className:"h-8",children:[e.jsx(c,{value:"file",className:"text-xs h-7",children:"檔案執行"}),e.jsx(c,{value:"inline",className:"text-xs h-7",children:"複製貼上"})]})})]}),e.jsxs(p,{onClick:P,className:"w-full gap-2",disabled:d.length===0,type:"button",children:[e.jsx(q,{size:16,weight:"fill"}),"生成腳本"]}),i&&e.jsxs(A.div,{initial:{opacity:0,y:10},animate:{opacity:1,y:0},className:"space-y-2",children:[e.jsx(m,{className:"text-sm font-semibold",children:"生成的腳本"}),e.jsx(L,{className:"h-64 w-full rounded-lg border border-border bg-muted/30",children:e.jsx("pre",{className:"p-4 text-xs font-mono",children:e.jsx("code",{children:i})})}),e.jsxs("div",{className:"flex gap-2",children:[e.jsxs(p,{onClick:j,variant:"outline",className:"flex-1 gap-2",type:"button",children:[e.jsx(K,{size:16}),"複製腳本"]}),e.jsxs(p,{onClick:y,className:"flex-1 gap-2",type:"button",children:[e.jsx(_,{size:16}),"下載腳本"]})]}),e.jsxs("div",{className:"p-3 bg-accent/10 rounded-lg border border-accent/20",children:[e.jsx("p",{className:"text-xs font-semibold text-accent-foreground mb-1",children:"使用說明:"}),e.jsx("p",{className:"text-xs text-muted-foreground whitespace-pre-line",children:X(r)})]})]})]})]})})}export{ne as AutomationDialog};
